...
DEFAULT_OUTPUT = ROOT / "fig" / "fig7_region_pesticide_boxplots.png"
DEFAULT_SUMMARY_OUTPUT = ROOT / "fig" / "fig7_region_pesticide_summary.csv"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot pesticide frequency and cost boxplots by region."
    )
    parser.add_argument(
        "--input",
        type=Path,
        default=DEFAULT_INPUT,
        help="CSV file containing regional pesticide observations.",
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def load_data(input_path: Path) -> pd.DataFrame:
    df = pd.read_csv(
        input_path,
        usecols=["Region", "Pest_Count", "Pest_Cost"],
    ).copy()
    df = df.rename(
        columns={
            "Region": "region_code",
    ...
    df["pest_count"] = pd.to_numeric(df["pest_count"], errors="coerce")
    df["pest_cost"] = pd.to_numeric(df["pest_cost"], errors="coerce")
    return df.dropna(subset=["region", "pest_count", "pest_cost"]).copy()


def build_summary_table(df: pd.DataFrame) -> pd.DataFrame:
    summary = (
        df.groupby("region", as_index=False)[["pest_count", "pest_cost"]]
...
def plot_boxplot(ax: plt.Axes, df: pd.DataFrame, column: str, y_label: str, tag: str) -> None:
    ...
    for pos, region, arr in zip(positions, regions, values):
        ...
        display_value = f"{mean_value:.1f}" if column == "pest_count" else f"{mean_value:.0f}"
        ax.text(
            0.02,
            0.84 - pos * 0.07,
